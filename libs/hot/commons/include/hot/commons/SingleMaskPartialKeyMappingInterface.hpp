...
public:
	inline uint64_t getExtractionMask() { return this->mSuccessiveExtractionMask; }
	// Our Work
	inline SingleMaskPartialKeyMapping(
		uint16_t mostSignificantBitIndex,
		uint16_t leastSignificantBitIndex,
		uint32_t offset,
		uint64_t extractionMask
	) : PartialKeyMappingBase(mostSignificantBitIndex, leastSignificantBitIndex),
		mOffsetInBytes(offset),
		mSuccessiveExtractionMask(extractionMask) {}
